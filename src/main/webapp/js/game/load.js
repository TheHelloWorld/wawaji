...
function load() {
    ...
    $(".game-back").css({'height':hc,"width":wc});
    ...
    zhuazi_size['di']['left'] = wc*0.13;//125

    current_size['di']['top'] = hc*0.61;//605
    current_size['di']['left'] = wc*0.13;//125
    ...
    //定义爪子三个部件的初始位置
    zhuazi_size['up']['left'] = wc*0.13;
    zhuazi_size['left']['left'] = wc*0.12;
    zhuazi_size['right']['left'] = wc*0.29;
    //获取初始top值
    ...
    current_size['up']['left'] = wc*0.13;
    current_size['left']['left'] = wc*0.12;
    current_size['right']['left'] = wc*0.29;
    //获取初始top值
...
function gameReturnMethod() {
    if(gameStatus == 1){
        $(".remindInfo").show();
        setTimeout(closeRemindInfo,1000);
        return;
    }
